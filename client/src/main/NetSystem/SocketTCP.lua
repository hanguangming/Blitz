...
local SOCKET_DATA_TICK_TIME = 0.05
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	if self.tcp == nil then
        self.tcp, errorStr = socket.tcp()
        if self.tcp == nil then
            printError("socket.tcp() return nil ", errorStr)
            return
        end
	end
...
function SocketTCP:_disconnect()
    ...
    DispatchEvent(GameEvent.SocketTCP_EVENT_CLOSED, {target = self})
end
...
function SocketTCP:_onConnected()
    ...
    print("SocketTCP:_onConnected", self.isConnected)
    DispatchEvent(GameEvent.SocketTCP_EVENT_CONNECTED, {target = self, isConnected = true})
	if self.connectTimeTickScheduler then scheduler.unscheduleGlobal(self.connectTimeTickScheduler) end
	...
	local __tick = function()
		while true do
            ...
            DispatchEvent(GameEvent.SocketTCP_EVENT_DATA, {data=(__partial or __body), partial=__partial, body=__body})
		end
	end

	-- start to read TCP data
    self.tickScheduler = scheduler.scheduleGlobal(__tick, SOCKET_DATA_TICK_TIME)
end

function SocketTCP:_connectFailure(status)
    DispatchEvent(GameEvent.SocketTCP_EVENT_CONNECT_FAILURE, {target = self, isConnected = false})
	self:_reconnect();
end
