----
-- 文件名称：SkillBuffPVP.lua
-- 功能描述：技能Buff copy from SkillBuff
-- 文件说明： 就目前技能表配置，
-- 作    者：王雷雷
-- 创建时间：2015-7-7
--  修改：
--
local SkillBuffPVP = class("SkillBuffPVP")
local SkillBuffTableDataManager = GameGlobal:GetDataTableManager():GetSkillBuffTableDataManager()
local SkillManager = nil
local CharacterManager = nil
local SkillBuffPVPManager = nil
...
function SkillBuffPVP:ctor(tableID, senderID, skillID, skillInstance, currentLevel)
    --当前Level
    self._CurrentLevel = currentLevel
    --当前
    self._SkillManager = self._CurrentLevel:GetSkillPVPManager()
    --当前
...
--初始化
function SkillBuffPVP:Init(skillInstance)
    local csbFileName = ""
    ...
    local currentSkill = self._SkillManager:GetSkill(self._SkillID)
    self._TargetIDList = {}
...
--销毁
function SkillBuffPVP:Destroy()
    self._TargetIDList = nil
end

--是否完成
function SkillBuffPVP:IsFinished()
    return self._IsFinished
end

--从目标列表移除
function SkillBuffPVP:RemoveTargetFromList(characterClientID)
...
--设置 ClientID
function SkillBuffPVP:SetClientID(clientID)
    self._ClientGUID = clientID
end
--保存角色当前的属性
function SkillBuffPVP:SaveCharacterProperty(currentCharacter)
...
--还原角色的属性
function SkillBuffPVP:RestoreCharacterProperty(currentCharacter)
    if self._AttributeType == BuffAttributeType.BuffAttributeType_HP then
...
--设置Buff的属性效果
function SkillBuffPVP:ApplyBuffProperty(currentCharacter)
    local addValue = 0
...
--更新
function SkillBuffPVP:Update(deltaTime)
    ...
    if self._CurrentContinueTime > self._ContinueTime then
        ...
        --还原作用目标的属性值
        for k, v in pairs(self._TargetIDList)do
            local currentCharacter = self._CharacterManager:GetCharacterByClientID(v)
                ...
                for k, v in pairs(self._TargetIDList)do
                    ...
                    if  currentCharacter ~= nil then
                        ...
                        if currentBuffInfo ~= nil then
                            local currentBuff = self._SkillBuffManager:GetSkillBuffByClientID(currentBuffInfo._BuffClientID) 
