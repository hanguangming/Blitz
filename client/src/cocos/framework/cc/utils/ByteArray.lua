...
function ByteArray:getZeroPosition()
    local pos = -1
    for i = self._pos, #self._buf do
        if self._buf[i] == string.char(0) then
            pos = i
            break
        end
    end
...
function ByteArray:readStringBytesZero()
    self:_checkAvailable()
    local zeroPosition = self:getZeroPosition()
    if zeroPosition == -1 then
        printError("readStringBytesZero zeroPosition == -1")
    end
    local len = zeroPosition - self._pos + 1
    local __, __v = string.unpack(self:readBuf(len), self:_getLC("z"))
    return __v
end

-- zero-terminated string
function ByteArray:writeStringBytesZero(__string)
    local __s = string.pack(self:_getLC("z"), __string)
    self:writeBuf(__s)
    return self
end
