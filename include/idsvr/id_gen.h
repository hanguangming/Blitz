...
#include "message.h"
#define ID_GEN_ACCOUNT 0x0
struct ID_Gen;
struct ID_GenReq : SSCC_REQUEST_BASE {
    static constexpr const char *the_class_name = "ID_GenReq";
    static constexpr int the_message_id = ID_GEN;
    static constexpr const char *the_message_name = "ID_GEN";
    typedef ID_Gen the_message_type;
    SSCC_UINT32 type;
    
    ID_GenReq(SSCC_ALLOCATOR_PARAM_DECL)
...
struct ID_GenRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "ID_GenRsp";
    static constexpr int the_message_id = ID_GEN;
    static constexpr const char *the_message_name = "ID_GEN";
    typedef ID_Gen the_message_type;
    SSCC_UINT64 id;
    
    ID_GenRsp(SSCC_ALLOCATOR_PARAM_DECL)
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT_INDENT(sscc_indent);
        SSCC_PRINT("id = ");
        SSCC_PRINT("%lu(0x%lx)", this->id, this->id);
        SSCC_PRINT(",\n");
    }
    void SSCC_DUMP_FUNC(const char *sscc_name, unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        if (!sscc_name) {
...
struct ID_Gen {
    static constexpr const char *the_class_name = "ID_Gen";
    static constexpr int the_message_id = ID_GEN;
    static constexpr const char *the_message_name = "ID_GEN";
    typedef ID_GenReq request_type;
    typedef ID_GenRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    ID_Gen(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
