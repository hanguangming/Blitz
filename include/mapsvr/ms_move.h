...
#include "message.h"
struct MS_Move;
struct MS_MoveReq : SSCC_REQUEST_BASE {
    static constexpr const char *the_class_name = "MS_MoveReq";
    static constexpr int the_message_id = MS_MOVE;
    static constexpr const char *the_message_name = "MS_MOVE";
    typedef MS_Move the_message_type;
    SSCC_UINT8 type;
    SSCC_VECTOR(SSCC_UINT32) path;
    
    MS_MoveReq(SSCC_ALLOCATOR_PARAM_DECL)
    : SSCC_REQUEST_BASE(SSCC_ALLOCATOR_PARAM),
      type(),
      path(SSCC_VECTOR(SSCC_UINT32)::allocator_type(SSCC_ALLOCATOR_PARAM))
    { }
    ...
    bool SSCC_SERIAL_FUNC(SSCC_SERIAL_PARAM_DECL) const override {
        ...
        if (!SSCC_WRITE_SIZE(SSCC_VECTOR_SIZE(this->path))) {
            return false;
        }
        for (auto &sscc_i : this->path) {
    ...
    bool SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM_DECL) override {
        ...
        do {
            ...
            for (size_t sscc_i = 0 ; sscc_i < sscc_size; ++sscc_i) {
                SSCC_VECTOR_EMPLACE_BACK(this->path);
                SSCC_READ_UINT32(SSCC_VECTOR_BACK(this->path));
            }
        } while (0);
        return true;
    }
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "path", 4);
        lua_createtable(sscc_L, SSCC_VECTOR_SIZE(this->path), 0);
        do {
            lua_Integer sscc_i = 0;
            for (auto &sscc_obj : this->path) {
                lua_pushinteger(sscc_L, ++sscc_i);
                SSCC_VECTOR_BACK(this->path);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        for (size_t sscc_i = 1; ; ++sscc_i) {
            ...
            do {
                int isnum;
                SSCC_VECTOR_BACK(this->path) = lua_tointegerx(sscc_L, -1, &isnum);
                if (!isnum) {
                    goto sscc_exit;
                }
            } while (0);
...
struct MS_MoveRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "MS_MoveRsp";
    static constexpr int the_message_id = MS_MOVE;
    static constexpr const char *the_message_name = "MS_MOVE";
    typedef MS_Move the_message_type;
    
    MS_MoveRsp(SSCC_ALLOCATOR_PARAM_DECL)
...
struct MS_Move {
    static constexpr const char *the_class_name = "MS_Move";
    static constexpr int the_message_id = MS_MOVE;
    static constexpr const char *the_message_name = "MS_MOVE";
    typedef MS_MoveReq request_type;
    typedef MS_MoveRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    MS_Move(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
