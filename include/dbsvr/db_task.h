...
#include "message.h"
struct DB_TaskUpdate;
struct DB_TaskUpdateReq : SSCC_REQUEST_BASE {
    static constexpr const char *the_class_name = "DB_TaskUpdateReq";
    static constexpr int the_message_id = DB_TASK_UPDATE;
    static constexpr const char *the_message_name = "DB_TASK_UPDATE";
    typedef DB_TaskUpdate the_message_type;
    SSCC_VECTOR(G_TaskOpt) task_opts;
    
    DB_TaskUpdateReq(SSCC_ALLOCATOR_PARAM_DECL)
    : SSCC_REQUEST_BASE(SSCC_ALLOCATOR_PARAM),
      task_opts(SSCC_VECTOR(G_TaskOpt)::allocator_type(SSCC_ALLOCATOR_PARAM))
    { }
    ...
    bool SSCC_SERIAL_FUNC(SSCC_SERIAL_PARAM_DECL) const override {
        ...
        if (!SSCC_WRITE_SIZE(SSCC_VECTOR_SIZE(this->task_opts))) {
            return false;
        }
        for (auto &sscc_i : this->task_opts) {
    ...
    bool SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM_DECL) override {
        ...
        do {
            ...
            for (size_t sscc_i = 0 ; sscc_i < sscc_size; ++sscc_i) {
                SSCC_VECTOR_EMPLACE_BACK(this->task_opts);
                if (!SSCC_VECTOR_BACK(this->task_opts).SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM)) {
                    return false;
                }
            }
        } while (0);
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT("task_opts = ");
        SSCC_PRINT("{\n");
        ++sscc_indent;
        do {
            size_t sscc_i = 0;
            for (auto &sscc_obj : this->task_opts) {
                SSCC_PRINT_INDENT(sscc_indent);
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "task_opts", 9);
        lua_createtable(sscc_L, SSCC_VECTOR_SIZE(this->task_opts), 0);
        do {
            lua_Integer sscc_i = 0;
            for (auto &sscc_obj : this->task_opts) {
                lua_pushinteger(sscc_L, ++sscc_i);
                SSCC_VECTOR_BACK(this->task_opts);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            for (size_t sscc_i = 1; ; ++sscc_i) {
                ...
                SSCC_VECTOR_EMPLACE_BACK(this->task_opts);
                if (!SSCC_VECTOR_BACK(this->task_opts).SSCC_FROMLUA_FUNC(sscc_L, -1 SSCC_FROMLUA_PARAM)) {
                    goto sscc_exit;
                }
                lua_pop(sscc_L, 1);
            }
...
struct DB_TaskUpdateRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "DB_TaskUpdateRsp";
    static constexpr int the_message_id = DB_TASK_UPDATE;
    static constexpr const char *the_message_name = "DB_TASK_UPDATE";
    typedef DB_TaskUpdate the_message_type;
    
    DB_TaskUpdateRsp(SSCC_ALLOCATOR_PARAM_DECL)
...
struct DB_TaskUpdate {
    static constexpr const char *the_class_name = "DB_TaskUpdate";
    static constexpr int the_message_id = DB_TASK_UPDATE;
    static constexpr const char *the_message_name = "DB_TASK_UPDATE";
    typedef DB_TaskUpdateReq request_type;
    typedef DB_TaskUpdateRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    DB_TaskUpdate(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
struct DB_TaskFinish;
struct DB_TaskFinishReq : SSCC_REQUEST_BASE {
    static constexpr const char *the_class_name = "DB_TaskFinishReq";
    static constexpr int the_message_id = DB_TASK_FINISH;
    static constexpr const char *the_message_name = "DB_TASK_FINISH";
    typedef DB_TaskFinish the_message_type;
    SSCC_VECTOR(G_TaskOpt) task_opts;
    SSCC_VECTOR(G_ValueOpt) value_opts;
    SSCC_VECTOR(G_BagItemOpt) item_opts;
    
    DB_TaskFinishReq(SSCC_ALLOCATOR_PARAM_DECL)
    : SSCC_REQUEST_BASE(SSCC_ALLOCATOR_PARAM),
      task_opts(SSCC_VECTOR(G_TaskOpt)::allocator_type(SSCC_ALLOCATOR_PARAM)),
...
struct DB_TaskFinishRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "DB_TaskFinishRsp";
    static constexpr int the_message_id = DB_TASK_FINISH;
    static constexpr const char *the_message_name = "DB_TASK_FINISH";
    typedef DB_TaskFinish the_message_type;
    
    DB_TaskFinishRsp(SSCC_ALLOCATOR_PARAM_DECL)
    : SSCC_RESPONSE_BASE(SSCC_ALLOCATOR_PARAM)
    { }
...
struct DB_TaskFinish {
    static constexpr const char *the_class_name = "DB_TaskFinish";
    static constexpr int the_message_id = DB_TASK_FINISH;
    static constexpr const char *the_message_name = "DB_TASK_FINISH";
    typedef DB_TaskFinishReq request_type;
    typedef DB_TaskFinishRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    DB_TaskFinish(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
