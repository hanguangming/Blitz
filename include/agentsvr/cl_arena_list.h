...
#include "message.h"
struct CL_ArenaList;
struct CL_ArenaListReq : INotify {
    static constexpr const char *the_class_name = "CL_ArenaListReq";
    static constexpr int the_message_id = CL_ARENA_LIST;
    static constexpr const char *the_message_name = "CL_ARENA_LIST";
    typedef CL_ArenaList the_message_type;
    
    CL_ArenaListReq(SSCC_ALLOCATOR_PARAM_DECL)
...
struct CL_ArenaListRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "CL_ArenaListRsp";
    static constexpr int the_message_id = CL_ARENA_LIST;
    static constexpr const char *the_message_name = "CL_ARENA_LIST";
    typedef CL_ArenaList the_message_type;
    SSCC_UINT32 self;
    SSCC_VECTOR(G_ArenaItem) tops;
    SSCC_VECTOR(G_ArenaItem) items;
    
    CL_ArenaListRsp(SSCC_ALLOCATOR_PARAM_DECL)
    : SSCC_RESPONSE_BASE(SSCC_ALLOCATOR_PARAM),
      self(),
      tops(SSCC_VECTOR(G_ArenaItem)::allocator_type(SSCC_ALLOCATOR_PARAM)),
      items(SSCC_VECTOR(G_ArenaItem)::allocator_type(SSCC_ALLOCATOR_PARAM))
    { }
    ...
    bool SSCC_SERIAL_FUNC(SSCC_SERIAL_PARAM_DECL) const override {
        ...
        if (!SSCC_WRITE_SIZE(SSCC_VECTOR_SIZE(this->tops))) {
            return false;
        }
        for (auto &sscc_i : this->tops) {
    ...
    bool SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM_DECL) override {
        ...
        do {
            ...
            for (size_t sscc_i = 0 ; sscc_i < sscc_size; ++sscc_i) {
                SSCC_VECTOR_EMPLACE_BACK(this->tops);
                if (!SSCC_VECTOR_BACK(this->tops).SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM)) {
                    return false;
                }
            }
        } while (0);
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "tops", 4);
        lua_createtable(sscc_L, SSCC_VECTOR_SIZE(this->tops), 0);
        do {
            lua_Integer sscc_i = 0;
            for (auto &sscc_obj : this->tops) {
                lua_pushinteger(sscc_L, ++sscc_i);
                SSCC_VECTOR_BACK(this->tops);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            for (size_t sscc_i = 1; ; ++sscc_i) {
                ...
                SSCC_VECTOR_EMPLACE_BACK(this->tops);
                if (!SSCC_VECTOR_BACK(this->tops).SSCC_FROMLUA_FUNC(sscc_L, -1 SSCC_FROMLUA_PARAM)) {
                    goto sscc_exit;
                }
                lua_pop(sscc_L, 1);
            }
...
struct CL_ArenaList {
    static constexpr const char *the_class_name = "CL_ArenaList";
    static constexpr int the_message_id = CL_ARENA_LIST;
    static constexpr const char *the_message_name = "CL_ARENA_LIST";
    typedef CL_ArenaListReq request_type;
    typedef CL_ArenaListRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    CL_ArenaList(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
