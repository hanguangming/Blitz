...
#include "message.h"
struct CL_UseHero;
struct CL_UseHeroReq : INotify {
    static constexpr const char *the_class_name = "CL_UseHeroReq";
    static constexpr int the_message_id = CL_USE_HERO;
    static constexpr const char *the_message_name = "CL_USE_HERO";
    typedef CL_UseHero the_message_type;
    SSCC_UINT32 sid;
    SSCC_INT8 use;
    
    CL_UseHeroReq(SSCC_ALLOCATOR_PARAM_DECL)
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT_INDENT(sscc_indent);
        SSCC_PRINT("use = ");
        SSCC_PRINT("%d(0x%x)", (int)this->use, (unsigned)this->use);
        SSCC_PRINT(",\n");
    }
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushinteger(sscc_L, (lua_Integer)this->use);
        lua_settable(sscc_L, sscc_index);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            do {
                int isnum;
                this->use = lua_tointegerx(sscc_L, -1, &isnum);
                if (!isnum) {
                    goto sscc_exit;
                }
            } while (0);
...
struct CL_UseHeroRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "CL_UseHeroRsp";
    static constexpr int the_message_id = CL_USE_HERO;
    static constexpr const char *the_message_name = "CL_USE_HERO";
    typedef CL_UseHero the_message_type;
    
    CL_UseHeroRsp(SSCC_ALLOCATOR_PARAM_DECL)
...
struct CL_UseHero {
    static constexpr const char *the_class_name = "CL_UseHero";
    static constexpr int the_message_id = CL_USE_HERO;
    static constexpr const char *the_message_name = "CL_USE_HERO";
    typedef CL_UseHeroReq request_type;
    typedef CL_UseHeroRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    CL_UseHero(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
