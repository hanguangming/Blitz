...
#include "message.h"
struct CL_Recast;
struct CL_RecastReq : INotify {
    static constexpr const char *the_class_name = "CL_RecastReq";
    static constexpr int the_message_id = CL_RECAST;
    static constexpr const char *the_message_name = "CL_RECAST";
    typedef CL_Recast the_message_type;
    SSCC_UINT32 item_id;
    SSCC_VECTOR(SSCC_UINT32) use_items;
    
    CL_RecastReq(SSCC_ALLOCATOR_PARAM_DECL)
    : INotify(SSCC_ALLOCATOR_PARAM),
      item_id(),
      use_items(SSCC_VECTOR(SSCC_UINT32)::allocator_type(SSCC_ALLOCATOR_PARAM))
    { }
    ...
    bool SSCC_SERIAL_FUNC(SSCC_SERIAL_PARAM_DECL) const override {
        ...
        if (!SSCC_WRITE_SIZE(SSCC_VECTOR_SIZE(this->use_items))) {
            return false;
        }
        for (auto &sscc_i : this->use_items) {
    ...
    bool SSCC_UNSERIAL_FUNC(SSCC_UNSERIAL_PARAM_DECL) override {
        ...
        do {
            ...
            for (size_t sscc_i = 0 ; sscc_i < sscc_size; ++sscc_i) {
                SSCC_VECTOR_EMPLACE_BACK(this->use_items);
                SSCC_READ_UINT32(SSCC_VECTOR_BACK(this->use_items));
            }
        } while (0);
        return true;
    }
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT(",\n");
        SSCC_PRINT_INDENT(sscc_indent);
        SSCC_PRINT("use_items = ");
        SSCC_PRINT("{\n");
        ++sscc_indent;
        do {
            size_t sscc_i = 0;
            for (auto &sscc_obj : this->use_items) {
                SSCC_PRINT_INDENT(sscc_indent);
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "use_items", 9);
        lua_createtable(sscc_L, SSCC_VECTOR_SIZE(this->use_items), 0);
        do {
            lua_Integer sscc_i = 0;
            for (auto &sscc_obj : this->use_items) {
                lua_pushinteger(sscc_L, ++sscc_i);
                SSCC_VECTOR_BACK(this->use_items);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            for (size_t sscc_i = 1; ; ++sscc_i) {
                ...
                do {
                    int isnum;
                    SSCC_VECTOR_BACK(this->use_items) = lua_tointegerx(sscc_L, -1, &isnum);
                    if (!isnum) {
                        goto sscc_exit;
                    }
                } while (0);
...
struct CL_RecastRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "CL_RecastRsp";
    static constexpr int the_message_id = CL_RECAST;
    static constexpr const char *the_message_name = "CL_RECAST";
    typedef CL_Recast the_message_type;
    
    CL_RecastRsp(SSCC_ALLOCATOR_PARAM_DECL)
...
struct CL_Recast {
    static constexpr const char *the_class_name = "CL_Recast";
    static constexpr int the_message_id = CL_RECAST;
    static constexpr const char *the_message_name = "CL_RECAST";
    typedef CL_RecastReq request_type;
    typedef CL_RecastRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    CL_Recast(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
