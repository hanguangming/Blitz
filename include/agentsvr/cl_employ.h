...
#include "message.h"
struct CL_Employ;
struct CL_EmployReq : INotify {
    static constexpr const char *the_class_name = "CL_EmployReq";
    static constexpr int the_message_id = CL_EMPLOY;
    static constexpr const char *the_message_name = "CL_EMPLOY";
    typedef CL_Employ the_message_type;
    SSCC_UINT32 sid;
    SSCC_INT8 employ;
    
    CL_EmployReq(SSCC_ALLOCATOR_PARAM_DECL)
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT_INDENT(sscc_indent);
        SSCC_PRINT("employ = ");
        SSCC_PRINT("%d(0x%x)", (int)this->employ, (unsigned)this->employ);
        SSCC_PRINT(",\n");
    }
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "employ", 6);
        lua_pushinteger(sscc_L, (lua_Integer)this->employ);
        lua_settable(sscc_L, sscc_index);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            do {
                int isnum;
                this->employ = lua_tointegerx(sscc_L, -1, &isnum);
                if (!isnum) {
                    goto sscc_exit;
                }
            } while (0);
...
struct CL_EmployRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "CL_EmployRsp";
    static constexpr int the_message_id = CL_EMPLOY;
    static constexpr const char *the_message_name = "CL_EMPLOY";
    typedef CL_Employ the_message_type;
    
    CL_EmployRsp(SSCC_ALLOCATOR_PARAM_DECL)
...
struct CL_Employ {
    static constexpr const char *the_class_name = "CL_Employ";
    static constexpr int the_message_id = CL_EMPLOY;
    static constexpr const char *the_message_name = "CL_EMPLOY";
    typedef CL_EmployReq request_type;
    typedef CL_EmployRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    CL_Employ(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
