...
#include "message.h"
struct CL_ArenaChallenge;
struct CL_ArenaChallengeReq : INotify {
    static constexpr const char *the_class_name = "CL_ArenaChallengeReq";
    static constexpr int the_message_id = CL_ARENA_CHALLENGE;
    static constexpr const char *the_message_name = "CL_ARENA_CHALLENGE";
    typedef CL_ArenaChallenge the_message_type;
    SSCC_UINT32 uid;
    
    CL_ArenaChallengeReq(SSCC_ALLOCATOR_PARAM_DECL)
...
struct CL_ArenaChallengeRsp : SSCC_RESPONSE_BASE {
    static constexpr const char *the_class_name = "CL_ArenaChallengeRsp";
    static constexpr int the_message_id = CL_ARENA_CHALLENGE;
    static constexpr const char *the_message_name = "CL_ARENA_CHALLENGE";
    typedef CL_ArenaChallenge the_message_type;
    SSCC_UINT32 award;
    ...
    CL_ArenaChallengeRsp(SSCC_ALLOCATOR_PARAM_DECL)
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT_INDENT(sscc_indent);
        SSCC_PRINT("award = ");
        SSCC_PRINT("%u(0x%x)", (unsigned)this->award, (unsigned)this->award);
        SSCC_PRINT(",\n");
        SSCC_PRINT_INDENT(sscc_indent);
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "award", 5);
        lua_pushinteger(sscc_L, (lua_Integer)this->award);
        lua_settable(sscc_L, sscc_index);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            do {
                int isnum;
                this->award = lua_tointegerx(sscc_L, -1, &isnum);
                if (!isnum) {
                    goto sscc_exit;
                }
            } while (0);
...
struct CL_ArenaChallenge {
    static constexpr const char *the_class_name = "CL_ArenaChallenge";
    static constexpr int the_message_id = CL_ARENA_CHALLENGE;
    static constexpr const char *the_message_name = "CL_ARENA_CHALLENGE";
    typedef CL_ArenaChallengeReq request_type;
    typedef CL_ArenaChallengeRsp response_type;
    
    SSCC_POINTER(request_type) req;
    SSCC_POINTER(response_type) rsp;
    
    CL_ArenaChallenge(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)), rsp() { }
};
