...
#include "message.h"
struct AS_MExp;
struct AS_MExpReq : SSCC_REQUEST_BASE {
    static constexpr const char *the_class_name = "AS_MExpReq";
    static constexpr int the_message_id = AS_MEXP;
    static constexpr const char *the_message_name = "AS_MEXP";
    typedef AS_MExp the_message_type;
    SSCC_UINT32 mexp;
    
    AS_MExpReq(SSCC_ALLOCATOR_PARAM_DECL)
    ...
    void SSCC_DUMP_FUNC(unsigned sscc_indent SSCC_DUMP_PARAM_DECL) override {
        ...
        SSCC_PRINT("mexp = ");
        SSCC_PRINT("%u(0x%x)", (unsigned)this->mexp, (unsigned)this->mexp);
        SSCC_PRINT(",\n");
    }
    ...
    void SSCC_TOLUA_FUNC(lua_State *sscc_L, int sscc_index) override {
        ...
        lua_pushlstring(sscc_L, "mexp", 4);
        lua_pushinteger(sscc_L, (lua_Integer)this->mexp);
    ...
    bool SSCC_FROMLUA_FUNC(lua_State *sscc_L, int sscc_index SSCC_FROMLUA_PARAM_DECL) override {
        ...
        if (!lua_isnil(sscc_L, -1)) {
            do {
                int isnum;
                this->mexp = lua_tointegerx(sscc_L, -1, &isnum);
                if (!isnum) {
                    goto sscc_exit;
                }
            } while (0);
...
struct AS_MExp {
    static constexpr const char *the_class_name = "AS_MExp";
    static constexpr int the_message_id = AS_MEXP;
    static constexpr const char *the_message_name = "AS_MEXP";
    typedef AS_MExpReq request_type;
    typedef void response_type;
    
    SSCC_POINTER(request_type) req;
    
    AS_MExp(SSCC_MESSAGE_PARAM_DECL) : req(SSCC_CREATE(request_type)) { }
};
